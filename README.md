...
Inside a transaction input cells contain their in-transaction values (i.e. the
effects of `swap!` and `reset!` are visible inside the transaction) but since
propagation does not occur during a transaction formula cells contain their
