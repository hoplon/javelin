...
(defn run-tests* []
  (when (< 0 (apply max ((juxt :fail :error) (run-tests))))
    (throw (ex-info "some test(s) failed" {}))))
   ...
   (testing "cell-let binding to formula"
     (let [a (cell [1 2])]
       (cell-let [[x y z] (cell= (map inc a))]
         ...
         (is (= @y 3))
         (is (= @z nil)))))
   (testing "cell-let binds map destruction"
     (let [m (cell {:x 1 :y 2})]
       (cell-let [{x :x y :y} m]
         (is (every? cell? [x y z]))
         (is (= @x 1))
         (is (= @y 2))
         (is (= @z nil)))))
   (testing "cell-let binds map destruction with :keys"
     (let [m (cell {:x 1 :y 2})]
       (cell-let [{:keys [x y]} m]
         (is (every? cell? [x y z]))
         (is (= @x 1))
